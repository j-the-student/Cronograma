...
onAuthStateChanged(auth,async user=>{if(!user){if(!location.pathname.endsWith("login.html"))location.href="login.html";return;}currentUser=user;document.getElementById("userEmail")&&(document.getElementById("userEmail").textContent=user.email||"");setupTheme();setupMenu();setupModals();setupLogout();setupNotifications();if(location.pathname.endsWith("index.html")||location.pathname.endsWith("/")){setupHomeForm(user);await loadHome(user);}});
